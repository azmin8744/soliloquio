...
The application uses a fixed-height, full-viewport layout divided into three
vertical sections.
...
- **Bottom Section:** * **User Profile:** Displays the current user's email
  fetched via the `me` query.
  - **Logout Button:** Triggers the `logout` mutation with the current
    `refreshToken`. On success, redirect to `/login`.
...
- **Behavior:** * Clicking a title updates `activePostId`.
  - **Guard:** If `isDirty.value` is true, the UI must prompt the user or
    auto-save via `updatePost` before switching.
...
- **Pane A (Markdown Editor):** Monospaced textarea binding to `editorBuffer`
  (`title` and `markdownContent`).
- **Pane B (Live Preview):** Toggled pane rendering the `content` (HTML) field
  from the backend.
...
export interface Post {
  id: string;
  [cite_start]; // UUID [cite: 3, 10]
  title: string;
  isPublished: boolean;
  markdownContent: string;
  content: string; // HTML
  updatedAt?: string;
  [cite_start]; // NaiveDateTime [cite: 5]
}
...
- **Logic:** If the `me` query or initial data fetch returns an `AuthError` or
  `401`, the BFF will attempt a `refreshAccessToken`.
...
- **Action:** Show a "Session Expired" notification. Redirect to `/login`. The
  implementation should attempt to save `editorBuffer` to `localStorage` as a
  fallback.
