...
impl RequiresAuth for ChangePasswordMutation {}
...
impl ChangePasswordMutation {
    async fn change_password(
        ...
        let current_user = match self.require_authenticate_as_user(ctx).await {
            Ok(user) => user,
            Err(e) => {
                return Ok(ChangePasswordResult::AuthError(AuthError {
                    message: e.to_string(),
                }));
            }
        };
        ...
        if let Err(validation_errors) = input.validate() {
            return Ok(ChangePasswordResult::ValidationError(ValidationErrorType {
        ...
        if let Err(e) = UserRepository::update_password(db, user_id, new_password_hash).await {
            return Ok(ChangePasswordResult::DbError(DbError { message: e.to_string() }));
        }
        ...
        Ok(ChangePasswordResult::PasswordChangeSuccess(PasswordChangeSuccess {
